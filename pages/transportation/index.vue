<template>
  <div>
    <h1>城市探索 - A7重劃區 交通指南</h1>
    <div class="flex flex-row flex-wrap">
      <div class="w-full md:w-1/2 p-2 shadow-md">
        桃園公車資訊
        <a href="https://ebus.tycg.gov.tw/ebus/driving-map/2020"
          >202 路線圖及時刻表</a
        >
        <NuxtImg
          src="https://kgptltaqnkidtcqkathb.supabase.co/storage/v1/object/public/a7-images/transportation/TRA-202.jpeg"
          alt="公車202"
        />
      </div>
      <div class="w-full md:w-1/2 p-2 shadow-md">
        桃園公車資訊
        <a href="https://ebus.tycg.gov.tw/ebus/driving-map/6030"
          >603 路線圖及時刻表</a
        >
        <NuxtImg
          src="https://kgptltaqnkidtcqkathb.supabase.co/storage/v1/object/public/a7-images/transportation/TRA-603.jpeg"
          alt="公車202"
        />
      </div>
      <div class="w-full md:w-1/2 p-2 shadow-md">
        桃園公車資訊
        <a href="https://ebus.tycg.gov.tw/ebus/driving-map/6060"
          >606 路線圖及時刻表</a
        >
        <NuxtImg
          src="https://kgptltaqnkidtcqkathb.supabase.co/storage/v1/object/public/a7-images/transportation/TRA-606.jpeg"
          alt="公車606"
        />
      </div>
      <div class="w-full md:w-1/2 p-2 shadow-md">
        桃園公車資訊
        <a href="https://ebus.tycg.gov.tw/ebus/driving-map/6052"
          >607 路線圖及時刻表</a
        >
        <NuxtImg
          src="https://kgptltaqnkidtcqkathb.supabase.co/storage/v1/object/public/a7-images/transportation/TRA-607.jpeg"
          alt="公車607"
        />
      </div>
    </div>
  </div>
</template>

<script setup>
useHead({
  title: "交通指南",
  meta: [
    {
      name: "description",
      content: "A7新林口-社區網站:  城市探索 - 交通指南 ",
    },
  ],
});
</script>

<style scoped></style>
